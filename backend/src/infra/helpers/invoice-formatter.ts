import { FormattedEnergy, FormattedInvoice } from '../../data/protocols/dtos/invoice'
...
type MatchEnergiesResponse = {
  energia_eletrica: FormattedInvoice['energia_eletrica']
  energia_injetada: FormattedInvoice['energia_injetada']
  en_comp_sem_icms: FormattedInvoice['en_comp_sem_icms']
}

export class InvoiceFormatter implements IInvoiceFormatter {
  format(input: string): FormattedInvoice {
    ...
    const invoice: FormattedInvoice = {
      ...invoiceMatch,
      ...energiesMatches,
  ...
  private matchEnergy(input: string, regex: RegExp): FormattedEnergy {
